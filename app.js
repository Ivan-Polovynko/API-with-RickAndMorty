class Store {
  ...
  setState(id, entity) {
    ...
    localStorage.setItem(id, JSON.stringify(Object.assign({}, state, { [entity.id]: entity })));
  }
}
  ...
  constructor(...args) {
    ...
    this.search.addEventListener("click", () => {
      if (this.inpValidaite(+this.input.value)) {
        this.request(`https://rickandmortyapi.com/api/character/${this.input.value}`);
      } else {
  ...
  onReload() {
    ...
    if (arr !== null) {
      for (const item of Object.values(arr)) {
        this.requestOnReload(`https://rickandmortyapi.com/api/character/${item.obj.id}`);
      }
    }
  }
